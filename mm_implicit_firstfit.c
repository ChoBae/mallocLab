...
static void *find_fit(size_t adjustedSize){
    ...
    for (bp = (char *)heap_listp; GET_SIZE(HDRP(bp)) > 0; bp = NEXT_BLKP(bp))   {
        ...
        if ((!GET_ALLOC(HDRP(bp))) && (adjustedSize <= GET_SIZE(HDRP(bp))))   {
            return bp;
        }
    }
...
void *mm_malloc(size_t size){
    ...
    extendSize = MAX(adjustedSize, CHUNKSIZE); // 최소 16byte 보장
